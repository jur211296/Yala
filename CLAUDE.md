...
| DraftBuilder | App/Services/DraftBuilder.swift | Helpers compartidos para mapear ParsedTransaction → InboxDraft (Vision) o ChatTransactionDraft (chat). findAccount/suggestSubcategory/matchSubcategoryByHint/matchTags/computeNeedsUserInput/build |
| ChatIntentClassifierService | Services/Chat/ChatIntentClassifierService.swift | Classifier `gpt-4.1-nano` ask/register/ambiguous + threshold 0.7 + regex fallback ES/EN |
...
| SankeyFlowCalculator | App/Logic/Calculators/SankeyFlowCalculator.swift | Widget Sankey (4 columnas: ingreso subcats → Gastos/Planificados/Disponible → gasto cats → gasto subcats). Puro, O(N). Rama Planificados (unified/byKind) + propagación opcional a categorías |
| PlannedOccurrenceBuilder | App/Logic/Calculators/PlannedOccurrenceBuilder.swift | Construye `[PlannedOccurrence]` para la rama Planificados del Sankey: enumera ocurrencias de SP activos en interval, excluye pagados (cruce con `tx.scheduledPaymentID` + day-match) y skipped, convierte multi-currency. Puro, no @MainActor |
...
- [2026-04-28] Traducción zh-Hans real keys-por-keys reemplaza rule-based v0 (commit `d6697ec5`). Tercer locale tras nl `b3f16aba` y pl `d6b3748e` en la misma sesión 2026-04-28; queda solo ja-JP por reemplazar. **Approach validado idéntico**: ~17 Edits GRANDES (~50-150 keys cada uno) cubriendo 91 secciones MARK + ~30 keys huérfanas inline; chunks de 500-700 líneas para Read; `grep -c '^"' file` cada Edit para detectar pérdidas; sesión completa ~60 min. **Decisiones específicas zh-Hans**: (1) **Pronombre informal `你`** consistente en todo (NUNCA `您` formal — Yala es app personal/conversacional). (2) **Measure words contextuales**: `笔` para transacciones (`%d 笔交易`, `5 笔活跃付款`), `天` para días (`最近 30 天`, `30 天`), `条` para registros, `次` para veces, `张` para tarjetas, `项` para items formales, `个月` para meses (siempre con `个`). (3) **Espaciado obligatorio chino-latín** según convención iOS Apple: `5 笔` no `5笔`, `Yala 应用` no `Yala应用`, `%d 个新通知` con espacios alrededor de placeholder. (4) **Punctuación china OBLIGATORIA en oraciones chinas**: `,` `。` `!` `?` `「」` `:` (NO `,` `.` `!` `?` inglesas). (5) **Weekday short usa calendario chino estándar**: `日/一/二/三/四/五/六` (1 carácter) en lugar de S/M/T/W/T/F/S del inglés que colisionaría visualmente. (6) **Currency invariables**: chino no pluraliza — `美元`/`欧元`/`元` sirven para 1 y 100. (7) **Glosario financiero canónico**: 账户/记录/支出/收入/预算/类别/子类别/标签/金额/日期/交易/计划付款/群组/总计/余额/货币/收据/订阅/设置/个人资料/可用/已支出/剩余. (8) **Frases motivacionales rephrased nativamente**: "You're crushing it!" → "你做得太棒了!", "A few minutes a day works wonders" → "每天几分钟,效果显著", "Way to go!" → "做得好!". **Issue placeholder corregido durante validación**: `accessibility.budgetRow %@ %d %@ %@` inicialmente usé `%d%%` (porcentaje literal) que el regex `%(?:\d+\$)?[@dlf]|%%` cuenta como placeholder extra → cambiado a `%d 百分比` para preservar exactamente los 4 placeholders del original (%@/%d/%@/%@). **Riesgos zh-Hans específicos validados con scripts**: (a) **Caracteres tradicionales (繁體) — 0 detectados** con script de detección de 50+ mapeos (個→个, 處→处, 這→这, 說→说, 見→见, 間→间, 們→们, 後→后, 體→体, 無→无, 為→为, 對→对, 應→应, 從→从, 當→当, 經→经, 記→记, 錄→录, 類→类, 別→别, 預→预, 帳→账, 戶→户, 設→设, 備→备, 訂→订, 閱→阅, 貨→货, 幣→币, 餘→余, 額→额, 網→网, 絡→络, 請→请, 確→确, 認→认 y más). Vulnerabilidad común si copias de fuentes Taiwan/HK. (b) **Mojibake encoding — UTF-8 charset confirmado** con `file -I`. (c) **Yala preservado en latín** 107 ocurrencias, NUNCA transliterado a 雅拉/亚拉/耶拉/押拉. **Validaciones verde**: paridad `en=3010 zh-Hans=3010 missing=0 extra=0` (3010 unique; **31 duplicados pre-existentes preservados**); placeholders 0 mismatches; NEEDS_TRANSLATION = 0; 3016 líneas con caracteres chinos (>5000 chars total); `xcodebuild test` 3 suites verde (LocalizationParityTests + StringsdictParityTests + BundleLocaleDriftTests). **Cambios**: Yala/Resources/zh-Hans.lproj/Localizable.strings (3,041 keys), YalaWidgets/Resources/zh-Hans.lproj/Localizable.strings (115 keys). stringsdict (regla `other` única — chino no diferencia singular/plural) y InfoPlist sin cambios. **Pendiente**: device QA visual en simulador con scheme Yala Dev (cambiar idioma a 简体中文 → recorrer Panel + Records + Profile + Statistics + NewTransaction; verificar visualmente: ningún `?` ni `□` (mojibake), ningún caracter tradicional, espaciado correcto chino-latín, punctuación china); siguiente locale ja-JP (último rule-based v0 sin reemplazar — prompt ya optimizado en commit `63bfed09`).
- [2026-04-28] 3 fixes runtime logs post-QA chat-registrar-transacciones (commits `773fe651` + `35bd11d1` + `aade3042`). Tras device QA del feature `chat-registrar-transacciones-yala-ia` el user reportó 3 issues de logs runtime. **Bug 1 (sheet collision chat→NTV)** — gate del consumer `.panel` mientras `showChatSheet=true` vía `markUnready/markReady` reusando el mecanismo del `AppRouter` (mismo patrón que `ContentView:350-359` usa para splash/biometric/wipe). El intent `presentNewTransactionFromChatDraft` queda en queue mientras chat abierto y drena al cerrar. Defensa en profundidad: guard `!sheets.showChatSheet` en `drainPanelIfActive` cubre cold start con deep link al chat. Los 8 intents `.panel` quedan correctamente bloqueados durante chat — ninguno tiene caso de uso legítimo simultáneo. **Bug 2 (ProfileImageStorage)** — guard `FileManager.default.fileExists(atPath:)` antes de `Data(contentsOf:)` distingue "archivo no existe" (caso normal, return silencioso) de "error real" (log con copy "Error loading existing file"). Comportamiento exterior idéntico (`imageData=nil` para users sin foto). **Bug 3 (UserSegmentService race)** — hallazgo más serio que el log original sugería. El `currentSegment` se leía como `.dormant` durante cold start porque `recalculate()` corría antes del primer `importEvent` de CloudKit (`tx=0`). `AppBootstrapper:555` usaba ese valor para auto-aceptar grupos compartidos sin confirmación → power-user con deep link en los <3s del cold launch veía su grupo aceptado silenciosamente. **Fix con NotificationCenter** (patrón establecido del codebase, verificado: `iCloudSyncService:99,120`, `PreferenceSyncService:103,184,308,328`, `AppPreferences:689,699`). `iCloudSyncService` añade flag `hasCompletedFirstImport` + `Notification.Name.iCloudFirstImportCompleted` post tras primer importEvent (idempotente). `UserSegmentService` añade `hasRecalculatedAfterFirstImport` + `firstImportObserver` registrado en `setContext` con cleanup defensivo del anterior. Helper privado `markRecalculatedAfterFirstImport()` evita duplicación entre callback observer y bloque cold-start race (cubre case "CloudKit ya completó antes de registrar observer"). `AppBootstrapper:555` gate-ea con la flag — si segmento no es confiable, cae a rama "require interaction" (más conservador, no destructivo). Log de `recalculate()` añade sufijo `(pre-import)` cuando flag false (mejora debugging post-mortem). **Decisión arquitectónica**: 2 flags separadas (no consolidadas en una sola en `iCloudSyncService`) — `hasCompletedFirstImport` significa "iCloud terminó", `hasRecalculatedAfterFirstImport` significa "Service ya recalculó tras el import"; conceptualmente distintas aunque ambas verdaderas tras el primer import. **Decisión scope**: línea 549 (caso onboarding incompleto) NO se toca — auto-acepta basado en flags de onboarding sin usar `currentSegment`, sin race. **Patrón observación**: codebase NO usa `Observations.values(of:)` ni `withObservationTracking` (verificado por exploración) — NotificationCenter es la convención establecida. **Test isolation gotcha**: tests del observer end-to-end (post + esperar callback + verificar flag) crashean con "signal trap" cuando corren paralelos a otros tests del suite por contaminación de singletons (`UserSegmentService.shared.recalculate()` toca `SessionState.shared.onboardingMode` + `iCloudSyncService.shared` simultáneamente con otros suites usando el mismo simulator clone). `@Suite(.serialized)` no resolvió porque xcodebuild despacha tests en paralelo a múltiples clones del simulator. Decisión: mantener solo el test `setContext_preImport_flagFalse` (verifica flag inicial, ese SÍ pasa); coverage del flow completo vendrá del **device QA con cold launch + cuenta poblada >100 tx** (verificar log `(pre-import)` en primer scan, sin sufijo en segundo post-import). **Simplify aplicó**: doc-comment de 4 líneas → 2 líneas conciso ("Gate critical decisions on this flag to avoid stale segment during cold start sync"); helper extraído `markRecalculatedAfterFirstImport()` evita duplicación entre observer y cold-start race; comentario trivial "queue: .main → callback en MainActor" eliminado; comentario histórico de bug en PanelShell de 4 líneas → 2 líneas (mantiene WHY conciso); línea en blanco doble accidental en iCloudSyncService eliminada. **Skipped en simplify** (false positives o fuera de scope): eliminar `fileExists` en ProfileImageStorage (ES el fix correcto del plan, no anti-pattern); consolidar las 2 flags en una sola (semánticamente distintas); refactor de `else if &&` multiline en AppBootstrapper a guard fail-fast (cambio de flow fuera de scope); reemplazar `if case` por `==` en test (ChatDraftPrefill tiene UUIDs aleatorios). **Tests**: 2 nuevos verdes (chatSheetUnreadyBlocksDrain_thenDrainsOnReady en AppRouterTests, setContext_preImport_flagFalse en UserSegmentServiceTests con `@Suite(.serialized)`). **Plan completo**: `~/.claude/plans/planificalo-bien-para-evitar-validated-tome.md`. **Pendiente**: device QA en Yala Dev de los 3 escenarios documentados en el plan (Edit en draft del chat → NTV abre sin warning; cold launch fresh → sin log "Error loading"; cold launch con cuenta poblada → log con sufijo `(pre-import)` en primer scan).
- [2026-04-27] Sankey con rama Planificados (`sankey-planned-branch`). El widget Sankey de Estadísticas → Distribución incluye ahora una rama **Planificados** entre Gastos y Disponible que resta el monto de los pagos planificados pendientes (recurrentes + suscripciones) del cash libre real. Mental model: el "Disponible" antes sobre-vendía cuánto cash quedaba; ahora muestra el saldo después de comprometidos. **Decisiones cerradas durante implementación**: (1) **Pureza preservada**: la conversión multi-currency vive en un nuevo `PlannedOccurrenceBuilder` (puro, sin `@MainActor`); el calculator solo recibe `[PlannedOccurrence]` con monto ya en preferred currency. Builder usa `CurrencyConverter.shared` (consistente con patrón existente). (2) **Aislamiento del cambio**: `compute(...)` añade 3 params con defaults (`plannedPending: [] = []`, `plannedSplit: .unified`, `propagatePlannedToCategories: false`) — pasar `[]` reproduce el output baseline byte-idéntico (regression-guarded por test). Rollback total = no llamar al builder en el VM. (3) **Edge case `pending > surplus`**: `plannedShown = min(plannedTotal, surplus)`, exceso silenciosamente colapsa Disponible a 0. Brand voice respetada: ningún regaño visual. (4) **Cruce con tx pagadas (evita doblar)**: builder construye `paidIndex: [String: Set<TimeInterval>]` one-pass desde `filteredTransactions` (`tx.scheduledPaymentID + Calendar.startOfDay`). Ocurrencia con tx mapeada → skip. Replica patrón de `ScheduledPaymentPaidStatusHelper` pero sobre interval arbitrario (no mes calendario). (5) **3 estados de visualización en una sola implementación**: `unified` (1 nodo), `byKind` (recurrentes + suscripciones split proporcional preservando ratio), `propagatePlannedToCategories` (links extra a expense cats existentes; cats sin gasto no se promueven — rama "colgada" en columna 1, intencional). El VM pasa `byKind: true, propagate: true` (estado validado visualmente por user). (6) **Filtro por `selectedAccounts`**: el builder respeta el filtro de cuentas del Sankey. Tags/currencies/needs no aplican a SP (no se etiquetan como tx). (7) **Reactividad SwiftUI**: `CategoriesTabView` añade `@Query var scheduledPayments` y un `scheduledPaymentsSignature: Int` (Hasher sobre id+amount+nextDueDate+paymentCategory+isActive+skippedDatesRaw de SPs activos) cableado a `.onChange` — cubre create/delete/edit/skip sin recompute spurious. (8) **Multi-currency**: SP con `currencyCode != defaultCurrencyCode` se convierte vía `convertWithLatestRate` (mismo patrón que `PanelViewModel`). (9) **Estructura de la View**: helper `recomputeSankey()` consolida los 4 callsites (onAppear + 3 onChange). (10) **L10n**: 3 keys × 6 locales (`statistics.sankey.planned`, `plannedRecurring`, `plannedSubscription`). PT/DE marcadas con `// TODO l10n review` para validación post-merge. (11) **Colors hardcoded** del calculator (`#A78BFA` planned, `#C084FC` plannedSubscription) siguen el patrón existente del módulo (`#FF0080` Gastos, `#6366F1` Disponible) — no DS.Semantic por consistencia local. (12) **Performance**: `paymentDatesInMonth` × meses tocados por interval × SPs activos. Para `.thisYear` con 50 SPs = ~600 invocaciones; instrumentación DEBUG print >50ms (removible al cerrar ticket). **Skipped por proporcionalidad** durante simplify: extraer `monthsTouched` a `Calendar+Extensions` (refactor moderado sin demanda inmediata), agrupar 3 params nuevos del calculator en un struct `PlannedConfig` (cosmético), añadir `predicate` al `@Query` de SP (filtrado in-memory es marginal). **Tests**: 20 nuevos verdes (12 SankeyFlowCalculator: cap, split, propagation, defaults regression-guard / 8 PlannedOccurrenceBuilder: paid exclusion, skip, income exclusion, multi-currency, interval cross-month). **Pendiente**: device QA con los 6 casos manuales (planned visible, paid no doble, exceso → Disponible 0, sin SP idéntico al baseline, periodo respeta interval, multi-currency convierte). Brand voice del label `"Planificados"` validado contra `L10n.Panel.scheduledTitle`. Estado VM final: `plannedSplit: .byKind, propagatePlannedToCategories: true` (Fase 3 completa).
- [2026-04-27] Chat → Registrar transacciones (commit `52d2ad6b`). Pipeline híbrido **Opción C** (classifier dedicado + parser dedicado + DraftBuilder local) sobre el flow context-rich previo (commit `ffc6ee59`). El chat antes era read-only; ahora detecta intent register y genera cards inline editables. Razones de la arquitectura: (1) **classifier `gpt-4.1-nano` sin contexto financiero** — barato (~$0.0001/call), evita exposición de data financiera para una clasificación que solo necesita el texto. (2) **parser `gpt-4.1-mini` con `subcategoryHint`** que el usuario ya usa en voice (`TranscriptionParserService.parseMultiple`). (3) **DraftBuilder helpers compartidos** entre voice (`VisionDraftFactory`) y chat — refactor de `findAccount/suggestSubcategory/computeNeedsUserInput` a `static func` reutilizables. **Cambios principales**: (1) `ChatTransactionDraft` struct in-memory con 5 status (pending/saving/saved/discarded/failed). Codable con `decodeIfPresent` para `tagIDs`/`savedTransactionID` (backward-compat con blobs persistidos). (2) `ChatAttachment` enum (`.drafts([...])` o `.ambiguous(canned, choices)`) en `ChatMessage` y `QAPair` con `decodeIfPresent`. `messageID?` añadido a `QAPair` para que `replaceDraft` localice por ID estable (matching por texto colisionaba en multi-tx con prefijos repetidos). (3) `ChatAssistantService.processQuestion` retorna `ChatAssistantResponse(text, attachments?, intent)`. **Guards comunes (daily limit + rate limit + counter increment) movidos al inicio**, antes del classifier branch — aplican a los 3 paths (ask/register/ambiguous). `forceIntent: ChatIntent?` permite a quick-reply chips bypassar classifier (evita reclasificar texto retórico). (4) `ChatIntentClassifierService` con few-shot ES+EN 12-15 ejemplos + threshold 0.7 (debajo → ambiguous) + regex fallback defensivo si LLM falla (patrones ES+EN; downgrade a `ask` para fr/de/it/pt — limitación documentada v1). (5) **`subcategoryHint` matching antes que MerchantMemory** (replica voice): `DraftBuilder.matchSubcategoryByHint` con normalize (case + diacritic insensitive) → exact → partial → reverse. (6) **Cap 5 multi-tx**: `parsed.prefix(5).map { build }` + nota inline en `text` si >5 ("Detecté N movimientos. Registro los primeros 5; manda el resto en otro mensaje"). NO segundo bubble (mantiene 1-question = 1-respuesta). (7) **Errores tipados en register flow**: parser fail por red/API propaga `ChatAssistantError.networkError` → banner del user; NO fallback silencioso a ambiguous (que confundía: el user dijo "registra X", la repregunta no aplicaba). (8) **Edit → NTV via RouterIntent**: `presentNewTransactionFromChatDraft(ChatDraftPrefill)` con `originIDs` + drain handler en PanelShell setea `sessionState.pendingChatDraftPrefill + showNewTransactionFromChat=true`. NTV usa overload nuevo `prefill(fromChatDraft:accounts:subcategories:)` (NO modifica el método existente que voice/image usan). Card invoca `@Environment(\.dismiss)` → cierra ChatSheet (agnóstico a los 3 puntos de entrada: Panel, Estadísticas, Records). (9) **Signal post-NTV-save sobrevive crash**: `SessionState.chatDraftSavedSignal` con `didSet` espejado a UserDefaults. Si la app se mata entre NTV-save y reabrir chat, ChatVM consume el signal persistido en `setContext` → marca card `.saved` con el `transactionID` real. **Sin esto, un segundo tap [Save] crearía duplicado**. (10) **Discard mantiene la card**: nuevo case `.discarded` en Status; row dimmed con strikethrough + opacity 0.55 + badge xmark. NO se remueve del attachment. (11) **`TransactionService.shared.setContext(context)` defensivo en `saveDraft`**: el bootstrapper no lo llama (otras vistas usan `context.insert/save` directo), por eso fallaba con "No ModelContext available" inicialmente. (12) **`context.model(for:)` O(1)** en lugar de `fetch(...) + filter` para resolver IDs en saveDraft. (13) UI: `TextField(value:format:)` con `Decimal?` Binding (rechaza input no parseable, sincroniza bidireccionalmente, sin `@State amountText` desync). Subcategorías agrupadas A-Z por categoría con `Section` en el Menu. `scrollDismissesKeyboard(.interactively)` en el ScrollView del chat (NO `dismissKeyboardOnTap` dentro del card — rompería los Menu hijos). Botón "Ver registros creados" tras al menos 1 save → `sessionState.navigateToRecordsStandalone()`. (14) **Mensajes "Registro creado/descartado" eliminados** post-feedback del user — las cards dimmed ya comunican el estado, evita ruido. **Out-of-scope v1**: modificar/borrar transacciones existentes desde chat (sub-ticket futuro `chat-correcciones-conversacionales`). Razón: requiere resolución de referencias conversacionales + tracking de cards saved + undo de transacciones + disambiguación con register; mini-feature de ~2 sesiones. **Tests**: 45 nuevos verdes (DraftBuilder 15, ChatIntentClassifier 19, ChatTransactionDraft 11). **Telemetría**: 5 eventos (chatIntentClassified, chatDraftProposed, chatDraftSaved, chatDraftDismissed, chatDraftEditedExternally, chatAmbiguousRepregunta). **L10n**: 17 keys × 6 idiomas = 102 strings. **Pendiente**: device QA de 27 escenarios CHR-01..CHR-27 en Yala Dev.
