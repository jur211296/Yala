...
//  canal de Grupos está apagado, crear un grupo se enruta a `.cloudKit` y tiene DOS finales, los dos
//  malos y el primero PERMANENTE — con cuenta iCloud del OS nace un grupo LOCAL con
//  `isBackendGroup = false` en silencio (y no hay ninguna llamada cliente a `migrate_group` en el
//  árbol, así que muere huérfano aunque el flag se encienda un minuto después: un grupo de una sola
//  persona, para siempre); sin ella, el seed de identidad lanza y el form pinta un `CKError` crudo.
//
