...
## Sale de qa · 2026-09-16 (barrido de QA) — espera un test, no verificación

- **Lo vivo es código:** la red de `financialMindset` que pide el criterio tras decidir que sincronice de
  verdad sigue sin escribir. Tiene que separar al escritor legítimo (`OnboardingView.swift:1718`) del eco
  (`PreferenceSyncService.swift:438`).
- **La mitad de device no tiene dónde mirarse:** ninguna pantalla enseña el iCloud KV ni el outbox, y
  los valores que subieron builds viejas siguen en el KV porque nada los borra. «Dejan de aparecer» no se
  puede comprobar tal como está escrito. Lo que sí está medido: las cinco claves ya son `synced: false`
  (`AppPreferences.swift:637`, `:665`, `:680`, `:687`, `:694`) y los `persist*` solo publican si
  `synced` es true (`:1249-1271`).
