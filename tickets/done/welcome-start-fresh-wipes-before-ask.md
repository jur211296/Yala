...
## QA Visual · 2026-09-16 — PASS (defecto 2)

Simulador iPhone 17 Pro (iOS 26.5), sobre `2.1` @ `bebd57a57`, `Yala Dev`. El 2026-08-14 quedó en PASS el defecto 1 y el 2 como no verificable; ahora
`-uitest-fail-wipe` lo hace verificable.

`-uitest-reset -uitest-seed grupos -uitest-fail-wipe` → Empezar → «Es mi primera vez» → «Borrar todo y
continuar» → alerta **«No pudimos borrar tus datos»** → OK → vuelve al selector y **no** entra al
onboarding. Control sin el arg: entra al onboarding con el nombre vacío.

Capturas: [alerta](../../qa/evidencia-barrido-20260916/21-welcome-fail-wipe-alert-no-pudimos-borrar.jpg) · [OK → selector](../../qa/evidencia-barrido-20260916/22-welcome-fail-wipe-ok-vuelve-al-selector.jpg).
