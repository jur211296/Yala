...
qa-date: 2026-09-16
qa-notes: No replicable (override Jurgen 2026-09-16) - AC 1 a 4 piden el estado de device-qa-private-gate-device-corpus, inalcanzable. AC 5 vive en welcome-private-fresh-start-skips-icloud-check recorrido 4
---
...
- **Criterios 1 a 4:** son los pasos del device-QA que este ticket mandó a
  `device-qa-private-gate-device-corpus`, cuyo estado de partida ya no se puede montar (cerrado hoy por lo
  mismo). La cadena literal del bug tampoco existe: `performPrivateReset` y
  `resetOnboardingFlagsPreservingData` dan 0 en `Yala/`, y el arreglo (#153, `8e3d476fb`) está en HEAD.
- **Criterio 5** («una instalación fresca sin datos no pregunta») es el **recorrido 4** de
  `welcome-private-fresh-start-skips-icloud-check`, que sigue en la cola de device.
- Los tres tickets que abrió ya existen en backlog.
