...
Simulador iPhone 17 Pro (iOS 26.5), sobre `2.1` @ `bebd57a57`, `Yala Dev`, `-uitest-reset -uitest-seed grupos`.

1. Empezar → «Es mi primera vez» → alerta «Empezar desde cero» → **Cancelar** → vuelve el selector
   entero (las tres tarjetas y «Volver»), no una pantalla en blanco. Tocar otra vez reabre la alerta.
2. Control: «Borrar todo y continuar» → onboarding con el nombre vacío.
3. Con `-uitest-fail-wipe`: «Borrar todo y continuar» → alerta **«No pudimos borrar tus datos»** → OK →
   vuelve al selector, sin entrar al onboarding.

Capturas: [Cancelar](../../qa/evidencia-barrido-20260916/20-welcome-cancelar-vuelve-al-selector.jpg) · [fallo del borrado](../../qa/evidencia-barrido-20260916/21-welcome-fail-wipe-alert-no-pudimos-borrar.jpg) · [OK → selector](../../qa/evidencia-barrido-20260916/22-welcome-fail-wipe-ok-vuelve-al-selector.jpg).
