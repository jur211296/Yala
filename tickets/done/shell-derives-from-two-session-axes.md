...
qa-date: 2026-09-16
qa-notes: No replicable (override Jurgen 2026-09-16) - exige un telefono con visita M1 creada por un build DEV anterior a 783a4ec9b, codigo ya borrado. Purga cubierta por SecondarySessionRetirementTests
---
...
- **El estado de partida no se puede producir hoy:** un teléfono con sesión de visita. TestFlight nunca pudo
  crearla (`SecondarySessionRetirement.swift:15-23`), y un Yala Dev solo pudo hasta `783a4ec9b` (13-sep),
  con el toggle DEBUG contra staging. Reproducirlo exige compilar código borrado y firmarlo en un iPhone.
- **Lo que queda de red:** la purga, con 11 tests en `SecondarySessionRetirementTests`. Todo el vocabulario
  de la visita da 0 en `Yala/` y `YalaWidgets/`.
- **Dos criterios sin marcar que no son de device**, por si los quieres en backlog: la celda E solo tiene
  unit (C, D y F tienen XCUITest), y no consta una pasada de la suite entera de XCUITest (el PR-B corrió 30
  tests en 10 suites).
