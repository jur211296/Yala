...
qa-date: 2026-09-16
qa-notes: No replicable (override Jurgen 2026-09-16) - solo-grupos ignora el wipe remoto desde el PR 157, no hay boton DEBUG y ningun otro camino deja mount neutro con filas en el Welcome
---
...
Las dos formas de montar el estado de partida están muertas, y no hay una tercera:

- **Opción A**, desde el PR #157: una sesión solo-grupos ya no obedece la señal de borrado remoto. Quien la
  detecta y quien la drena exigen sesión privada confirmada (`DestructiveScopeLogic.swift:179-181`,
  `RemoteWipeSignalDecider.swift:97-102`).
- **Opción B:** el botón DEBUG que emitiría la señal no existe (`CloudSyncDebugView.swift`); el único emisor
  es «Vaciar datos», y marca el propio teléfono para que no reaccione.
- **Ninguna otra vía** deja a la vez el store montado en neutro, filas dentro y el Welcome delante: «Vaciar
  datos» en solo-grupos aterriza en la pestaña de Grupos, «Cerrar sesión» borra el store antes de montar, y
  bajo `-uitest` la puerta no sale.

El único camino teórico pasa por el consentimiento de Grupos fallando en un segundo login, o sea, solo
existiría mientras hubiera un bug (ver `groups-consent-door-spec`).
