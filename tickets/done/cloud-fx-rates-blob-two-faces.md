...
qa-date: 2026-09-16
qa-notes: No replicable (override Jurgen 2026-09-16) - device afectado inexistente tras fresh start 2026-09-10, sintoma solo print DEBUG y la API reescribe doubles. Fix 56233a1f en HEAD con 6 tests decodedRates
---
...
- **Nunca hubo nada que ver en pantalla:** el síntoma era un `print` bajo `#if DEBUG`
  (`ExchangeRate.swift:81-83`), y las filas de julio del teléfono afectado desaparecieron con el fresh
  start del 2026-09-10.
- **Tampoco se puede fabricar hoy de forma que discrimine:** cualquier pull de la nube escribe la cara
  string (`EntityApplyMap.swift:369`), pero la precarga de tasas la reescribe con doubles en carrera
  (`ExchangeRateService.swift:80-99` y `:483-494`), así que un iPhone no distingue «se leyó bien» de «la
  API la pisó». En simulador ni siquiera hay pull personal sin App Attest (`CloudSyncRuntime.swift:531-539`).
- **Lo que queda de red:** el fix `56233a1f` está en HEAD, con el decode tolerante
  (`ExchangeRate.swift:59-86`) y seis tests `decodedRates_*` más `Canonc1CodecTests`.
