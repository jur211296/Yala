...
qa-status: passed
qa-date: 2026-09-16
qa-notes: Piezas 1, 2 y 3 canal apagado PASS en sim 2026-09-16. Copy de grupo legacy no replicable (legacy retirados y sin productor desde C4) - override Jurgen 2026-09-16
---
...
## QA Visual · 2026-09-16 — PASS (piezas 1, 2 y canal apagado)

Simulador iPhone 17 Pro (iOS 26.5), sobre `2.1` @ `bebd57a57`. Las tres cosas de «Qué falta ver»:

1. **El nombre del grupo en la bienvenida, con la app cerrada.** `Yala Dev` con el enlace completo pasado
   en frío (`-uitest-deeplink-url`, con `n`, `i` y `c`) → «Te invitaron al grupo **Viaje a Cusco**», con
   el avión y el círculo naranja.
2. **La forma mínima no borra la marca.** Matar la app y relanzar **sin reset** con la forma mínima (`g`
   y `t`) → sigue «Viaje a Cusco». Control: tras «Más tarde», la forma mínima en caliente sale genérica
   («Te invitaron a un grupo»). Declinar borra la marca; la forma mínima, no.
3. **Copy del canal apagado.** Build `Debug` sin `DEV_BUILD` (bajo `-uitest` el canal de Grupos vale
   OFF) + seed `grupos` → Viaje a Cusco → 3 miembros → «Invitar por enlace» → **«No podemos crear enlaces
   de invitación en este momento. Inténtalo de nuevo en un rato.»** Ya no culpa a la conexión.

**Copy de grupo legacy: cerrado sin verificar (override de Jürgen 2026-09-16).** No existe forma de
tener un grupo de la era CloudKit delante: `LegacyGroupsRetirement` los oculta y ninguno puede nacer
desde C4 (`ad291c7f`), así que ningún usuario llega a ese mensaje.

Capturas: [en frío](../../qa/evidencia-barrido-20260916/26-invite-frio-nombre-icono-color-y-mas-tarde.jpg) · [forma mínima](../../qa/evidencia-barrido-20260916/28-invite-forma-minima-en-frio-conserva-nombre.jpg) · [control en caliente](../../qa/evidencia-barrido-20260916/27-invite-caliente-onboarded-nombre-prellenado-mas-tarde.jpg) · [canal apagado](../../qa/evidencia-barrido-20260916/38-invite-canal-apagado-no-culpa-la-conexion.jpg).
