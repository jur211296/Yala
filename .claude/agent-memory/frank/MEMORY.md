...
- [Un límite de plataforma se explica en la pregunta](feedback_un_limite_de_plataforma_se_explica_en_la_pregunta.md) — preguntó por qué Apple solo usa el Apple ID del…

## Cómo mido y cómo entrego
- [El testigo se apunta ANTES de conducir](feedback_el_testigo_se_apunta_antes_de_conducir.md) — el kill de la 1.ª pasada; y la marca de sesión…
- [Un fail-closed sin reintento es permanente](feedback_un_fail_closed_sin_reintento_es_permanente.md) — el motor `.idle` que nadie re-evalúa: busca…
- [Un case nuevo hereda cada `!=` del repo](feedback_un_case_nuevo_hereda_cada_distinto_de.md) — `uiState != .idle` coló `.journalUnreadable`…
...
- [La huella se ata al que ENTRA, no a la respuesta](feedback_la_huella_se_ata_al_que_entra.md) — sellé en `existing_stable` y «Activar Yala…
- [El copy lo elige quien PRODUJO el motivo](feedback_el_copy_lo_elige_quien_produjo_el_motivo.md) — y la clasificación también; y el testigo se lee…
- [El estado compartido no es testigo de su rama](feedback_el_estado_compartido_no_es_testigo_de_su_rama.md) — un campo lo enciende 1 sitio y lo apagan 13…
- [Mi rollback quita lo que otros leían](feedback_mi_rollback_quita_lo_que_otros_leian.md) — un fetch ve lo sucio; deshacerlo cegó al guard…
- [Cortar el reintento corta la recuperación](feedback_cortar_el_reintento_corta_la_recuperacion.md) — el sello retirado no cortaba el bucle y dejaba al…
- [Mi arreglo quita la SALIDA que había](feedback_mi_arreglo_quita_la_salida_que_habia.md) — cerrar un abuso puede dejar sin puerta al dueño…
- [Cerrar un atajo quita lo que capturaba](feedback_cerrar_un_atajo_quita_lo_que_capturaba.md) — el ciclo sin candado era el único drain del History.
- [La limpieza «por si acaso» enmascara mutantes](feedback_la_limpieza_por_si_acaso_enmascara_mutantes.md) — una línea inalcanzable recoge lo que el camino…
- [Fundir una entrada nueva hereda sus excepciones](feedback_fundir_una_entrada_nueva_hereda_sus_excepciones.md) — un `||` en el parámetro de la puerta le regaló…
- [Un verbo nuevo hereda las PROHIBICIONES del viejo](feedback_un_verbo_nuevo_hereda_las_prohibiciones_del_viejo.md) — los negativos se anclan al nombre y el gemelo…
- [Sellar en la buena noticia cobra la app cerrada](feedback_sellar_en_la_buena_noticia_cobra_la_app_cerrada.md) — el seguidor sellado al entrar salía con un poll…
- [Un reinicio se mide contra su CADENCIA](feedback_un_reinicio_se_mide_contra_su_cadencia.md) — racha vs. acumulado: con el re-kick de 30 s…
- [El techo que se resetea no es un techo](feedback_el_techo_que_se_resetea_no_es_un_techo.md) — enumera quién lo reinicia y a quién muerde; con…
...
- [Dos derivados del mismo enum no son independientes](feedback_dos_derivados_del_mismo_enum_no_son_independientes.md) — comparten su forma de AGRUPAR; defenderlo por…
- [El scan de un modifier no ancla a QUÉ vista cuelga](feedback_el_scan_de_un_modifier_no_ancla_a_que_vista_cuelga.md) — moverlo un nivel hacia dentro reabre el ticket…
- [Al quitar un apagado incondicional, busca quién lo usaba](feedback_al_quitar_un_apagado_incondicional_busca_quien_lo_usaba.md) — condicionar un efecto se lo quita a caminos que…
- [La puerta de la unidad no cubre sus peticiones](feedback_la_puerta_de_la_unidad_no_cubre_sus_peticiones.md) — «antes de cada página» no cubre los trozos de…
...
- [El reintento de fuera multiplica el de dentro](feedback_el_reintento_de_fuera_multiplica_el_de_dentro.md) — el SDK ya reintentaba: mi coste era falso por…
...
- [El mutante muere y el término sobra](feedback_el_mutante_muere_y_el_termino_sobra.md) — 5 mutantes muertos y la rama solo la alcanzaba…
- [La prueba que sustituyo implicaba más](feedback_la_prueba_que_sustituyo_implicaba_mas.md) — el marcador implicaba «llegaron las…
- [Un registro no prueba la elección](feedback_un_registro_no_prueba_la_eleccion.md) — bloqueé el `nil` y un cinturón fabricaba el…
- [Si no sabes qué lado conoce el backend, emite las dos](feedback_si_no_sabes_que_lado_conoce_el_backend_emite_las_dos.md) — traducir al relevo iba al revés en el líder desplazado.
- [El copy que propongo hereda las prohibiciones del vecino](feedback_el_copy_que_propongo_hereda_las_prohibiciones_del_vecino.md) — recomendé «tus datos siguen aquí» y #221 lo…
...
- [El copy que manda a una pantalla, por esa pantalla](feedback_el_copy_que_manda_a_una_pantalla_se_clasifica_por_esa_pantalla.md) — clasifica por lo que ELLA deriva.
- [Antes de poner techo, mide que la espera existe](feedback_antes_de_poner_techo_mide_que_la_espera_existe.md) — acoté una espera que para casi todos terminaba al…
- [La pasada anterior cambia mi predicado](feedback_la_pasada_anterior_cambia_mi_predicado.md) — en un paso reintentable, lo que dejó la pasada fallida…
- [Una ventana dura lo que su reintento](feedback_una_ventana_dura_lo_que_su_reintento.md) — «una petición» era todo lo que el claim pasa…
- [Mi puerta bloquea lo que su propio flujo dejó](feedback_mi_puerta_bloquea_lo_que_su_propio_flujo_dejo.md) — el claim deja la cuenta `complete` y mi…
...
- [El escritor que imagina el ticket](feedback_el_escritor_que_imagina_el_ticket.md) — el líder que escribe a diario siempre dejó marcador; era un adoptador.
- [La premisa del ENCARGO también se mide](feedback_la_premisa_del_encargo_tambien_se_mide.md) — hereda los errores del ticket, también el CÓMO…
- [El paso de ANTES también lee](feedback_el_paso_de_antes_tambien_lee.md) — cerré seis inventarios y el backfill que los alimenta…
- [Una frase de alcance se cuenta por llamadores](feedback_una_frase_de_alcance_se_cuenta_por_llamadores.md) — «solo en carrera» cayó: la migración sube sin…
- [`-only-testing` filtra por TIPO, no por fichero](feedback_only_testing_filtra_por_tipo_no_por_fichero.md) — pedí 12 suites y corrieron 7. Cuenta pedidas…
...
- [El seam que lanza lo mismo no prueba el `catch`](feedback_el_seam_que_lanza_lo_mismo_no_prueba_el_catch.md) — que lance un `CocoaError`; si no, «seam fuera…
...
- [El `contains` deja sitio a una sentencia ANTEPUESTA](feedback_el_contains_deja_sitio_a_una_sentencia_antepuesta.md) — fija el cuerpo ENTERO; y cuenta el…
- [Los mutantes compilan solo YalaTests](feedback_mutantes_compilan_solo_yalatests.md) — sin `-only-testing` en el build, 15 min por mutante; con…
- [El script de mutantes revierte mi trabajo](feedback_el_script_de_mutantes_revierte_mi_trabajo.md) — restaura desde una COPIA del scratchpad, no con…
...
- [El denominador de una resta es el número que se ve](feedback_el_denominador_de_una_resta_es_el_numero_que_se_ve.md) — numerador en magnitudes, denominador en lo que…
...
- [Un arreglo en N sitios se prueba en los N](feedback_un_arreglo_en_n_sitios_se_prueba_en_los_n.md) — 1 test para 18 ramas dejó 34 sitios en verde…
...
- [Una marca que abre una entrada se ata a quien la ganó](feedback_una_marca_que_abre_una_entrada_se_ata_a_quien_la_gano.md) — la tarjeta de adopt sin puerta adoptaba la…
- [Mi salida nueva es un camino muerto](feedback_mi_salida_nueva_es_un_camino_muerto.md) — antes de cambiar «sigue» por «vuelve»; y cuenta…
- [El otro control va al mismo sitio](feedback_el_otro_control_va_al_mismo_sitio.md) — el chevron salía sin retirar el arm que su…
- [Mi arreglo abre un camino INALCANZABLE](feedback_mi_arreglo_abre_un_camino_inalcanzable.md) — quitar un efecto vuelve alcanzable código que…
...
- [El alert de SwiftUI compite con su propio botón](feedback_el_alert_de_swiftui_compite_con_su_propio_boton.md) — el `set` del binding corre al pulsar CUALQUIER…
...
- [Un `await` de red en el arranque bloquea la pantalla](feedback_un_await_de_red_en_el_arranque_bloquea_la_pantalla.md) — deja sin correr el `defer` que libera…
...
- [Mi refutación falla ABIERTO](feedback_mi_refutacion_falla_abierto.md) — 33 de 45 hallazgos salieron «refutados» sin que nadie los…
- [Tras la puerta, el error es otro](feedback_tras_la_puerta_el_error_es_otro.md) — copié «el 401 del attest suma a la racha» de Grupos al…
...
- [El tope del test de cancelación es PROPIO](feedback_el_tope_del_test_de_cancelacion_es_propio.md) — el mutante que deja la espera sin resolver la…
...
- [Apagar una voz destapa el residual de la otra](feedback_apagar_una_voz_destapa_el_residual_de_la_otra.md) — el spinner tapaba con una mentira el hueco del…
...
- [La rule de área es una lente más](feedback_la_rule_de_area_es_una_lente_mas.md) — leerla CONTRA el diff: cazó lo que las tres lentes no…
...
- [El orden del enum se ve fuera](feedback_el_orden_del_enum_se_ve_fuera.md) — un case nuevo en medio cambia el número del alert que se…
...
- [El mecanismo que reuso trae sus precondiciones](feedback_el_mecanismo_que_reuso_trae_sus_precondiciones.md) — si no las cumples, entra por el COORDINADOR que…
...
- [El reparador, tan re-ejecutable como el destructor](feedback_el_reparador_tan_reejecutable_como_el_destructor.md) — en un hook kill-safe, `consume()` mata lo que…
...
- [Avisar a Frank: lo hace el hook, no tú](reference_avisar_a_frank_webhook.md) — el hook manda PR y rojos solo; «prueba» en el texto lo…
...
- [`git` bloqueado por la licencia de Xcode](reference_git_bloqueado_por_licencia_de_xcode.md) — tras actualizar Xcode; el binario real del…
...
- [El disco: los GB están DENTRO del simulador](reference_disco_cache_simulador.md) — cachés regenerables del device; tras mutantes…