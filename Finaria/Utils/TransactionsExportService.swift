...
struct TransactionsExportService {
    ...
    static func exportToCSV(
        ...
        // Obtenemos el intervalo de fechas efectivo, si aplica.
        ...
        // Si el periodo es personalizado y hay algún dato de fecha pero
        // no se pudo construir un intervalo válido, devolvemos error específico.
        if filters.period == .custom,
           (filters.customDateFrom != nil || filters.customDateTo != nil),
           effectiveInterval == nil {
            throw TransactionsExportError.invalidCustomDateRange
        }
    ...
    /// Si se proporciona un `DateInterval`, lo aplica como filtro.
    private static func makeFetchDescriptor(
        ...
        // Si tenemos intervalo, construimos un predicado simple por fecha.
        if let interval = effectiveInterval {
    ...
    private static func writeCSVToTemporaryFile(data: Data) throws -> URL {
        ...
        let fileName = "Finaria_Transacciones_\(timestamp).csv"
