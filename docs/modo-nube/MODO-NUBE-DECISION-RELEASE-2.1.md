...
1. **El proyecto Supabase de PRODUCCIÓN pasa a ser el bloqueante #1 de 2.1.** Hoy `CloudBackendConfig.supabaseURL` y `anonKey` devuelven `nil`/`""` fuera de `DEV_BUILD` (`Yala/App/Services/CloudBackendConfig.swift:23-44`) ⇒ `isConfigured == false` ⇒ toda la superficie de nube y auth está inerte. **No hay 2.1 sin ese proyecto creado, con su schema desplegado y su PITR decidido.** Es infraestructura y coste recurrente, no código.
