...
>
> **La hipótesis de fix quedó CONFIRMADA con medición (2026-07-28, 18:40).** Proponen gatear el drenaje de `.showInboxAlert` por «arranque completado» en vez de solo «anchor libre». Pero el test **ya espera `uitest_ready` antes de descartar** y sigue cayendo 3/9. Ellos lo anticipan (esperar ready en el test retrasa el **tap**; gatear en la app retrasaría el **montaje** del cover, que es distinto), y es coherente con que `ShellReadinessState` ya exista — pero sigue siendo hipótesis. **RESULTADO del experimento:** se movió el submit del hook de media función de `applyUITestSeed` al call site (`AppBootstrapper.swift:470`), o sea **después** del `markReady()` que el `defer` de esa función ejecuta al retornar. **18 corridas en iOS 27.0: `test_inboxAlertModalDismisses` 18/18 verde y el fallo `:88` cero veces, contra 3/9 sin el cambio** (p ≈ 0,0007 si la tasa siguiera en el 33 %). ⇒ **la causa es el MOMENTO del montaje**, y el gate de producción está justificado. El caso pasa de 15-18 s a 19-25 s: espera extra del test, no degradación del producto.
>
> **TRAMPA que el fix de producción tiene que evitar, y la encontró el propio experimento:** mover solo el aviso **invierte la pareja aviso→paywall** (el `presentTrialOffer` seguía dentro del seed) ⇒ el paywall se encola primero, se monta encima del alert y rompe `PaywallInboxAlertRoutingUITests:35` — una línea que **no había fallado nunca** (los rojos previos de esa suite fueron `:45`, `:53`, `:51`). Con la pareja movida junta y en orden: 9/9 verde incluido el paywall. **El orden aviso → paywall es load-bearing para cualquier gate que retrase el drenaje.**
>
> **El experimento se REVIRTIÓ y NO se commiteó, a propósito.** Gatea el *hook de uitest*, no la ruta real: en producción el aviso lo encola `checkInboxDraftsAndNotify` desde el sync. Dejarlo puesto pondría la suite **verde sin arreglar el producto** — convertiría el único detector del bug en un falso negativo mientras el usuario sigue expuesto. Diff en `evidencia-2026-07-28/experimento-cover-tras-readiness.diff`.
>
...
- ~~**El pase tiene que hacerse en la Mac con SDK 27.0.**~~ **CORREGIDO el 2026-07-29 — era falso, y su razón se cae sola.** El argumento era «27.0 es el SDK contra el que se compilará 2.1», pero **la release NO se archiva en esa Mac**: su Xcode es una BETA (`27A5209h`) y App Store Connect rechaza los archives de Xcode beta ⇒ el archive sale de la Mac de Xcode **26.6 estable**, así que el SDK de release es **26.5**. Regla de reparto vigente: **por defecto TODO en la Mac de Xcode estable** (es la de release, tiene mejor CPU/disco y más presupuesto de tokens); la Mac de 27.0 **solo cuando sea indispensable**, y lo único indispensable es *ejercitar el runtime iOS 27.0* — que afecta a XCUITest y comportamiento de UI, **no** a las suites unitarias, que son indiferentes al runtime. Merece la pena pasar los XCUITest en 27.0 antes de una release, como red del iOS que viene: ya destapó dos bugs reales (ningún swipe sintético materializa una celda de `LazyVGrid`, y el cover de la bandeja montándose antes del bootstrap). Lo que SIGUE siendo cierto del párrafo original: Las dos Macs tienen Xcode distinto — ésta Xcode-beta/SDK 27.0, la otra Xcode 26.6/SDK 26.5, sin runtime 27.x instalable. ⇒ **ninguna receta con un runtime hardcodeado es portable**; comprobar con `xcodebuild -showBuildSettings | grep SDK_NAME`. Ya costó un chip mal escrito.
- ~~**`config_isConfigured_inTestScheme` cambia de expectativa en ese MISMO commit.**~~ **FALSO, corregido el 2026-07-30.** El test (`YalaTests/CloudSync/CloudAuthServiceTests.swift:51-56`) afirma `isConfigured == true`, y hoy falla con el scheme `Yala` PRECISAMENTE porque producción es un placeholder ⇒ el #4 lo pone **VERDE** en los dos schemes, no rojo. No hay expectativa que invertir; lo único desactualizado será el comentario de `:52` («los tests corren bajo Yala Dev → staging configurado»), que pasará a ser cierto en ambos. Probablemente apague también el otro rojo conocido (`secondaryEntry_killedByRemoteOff`) — comprobarlo, no darlo por hecho. Texto original, por si sirve de contexto: Hoy es rojo-conocido con el scheme `Yala` porque pinnea «producción no configurada»; al cablear las dos líneas pasa a rojo-NUEVO si no se actualiza a la vez.
...
| # | Qué | Estado |
|---|---|---|
| **0** | **BLOQUEANTE DE RELEASE 2.1 — el cover del aviso de bandeja se queda PEGADO y la app no responde.** En iOS 27.0, ~1 de cada 3 arranques: se descarta el aviso, el `fullScreenCover` no se desmonta y **el Panel ignora 10 taps a lo largo de ~40 s**. Es funcionalmente la «toolbar muerta» del 2.0.5 ya publicado, así que no es teórico: esa clase de fallo YA costó un incidente en producción. **Hoy el impacto directo se limita a quien esté en la beta de iOS 27, pero 2.1 seguirá viva cuando 27 salga en público** ⇒ hay que arreglarlo antes, no después. **Palanca CONFIRMADA con medición** (experimento del §3.2: 18 corridas en 27.0, 18/18 verde y `:88` cero veces contra 3/9 ⇒ la causa es el momento del montaje): gatear el drenaje de `.showInboxAlert` por «arranque completado» y no solo por «anchor libre», encajándolo en `ShellReadinessState`. **Dos condiciones que el fix debe respetar:** (a) mover la pareja **aviso → paywall junta y en ese orden** —invertirla rompe la serialización—, y (b) gatear la ruta REAL (`checkInboxDraftsAndNotify` desde el sync), no el hook de uitest: el experimento se revirtió justamente porque dejarlo puesto pondría la suite verde sin arreglar el producto. Reporte a Apple justificado por separado, pero no resuelve: un runtime beta no arregla el 2.0.5 publicado. **CERRADO 2026-07-28.** Fix en `66960f7d` (otra Mac): gatea la MATRIZ de readiness por «arranque completado» —`SessionState.isBootstrapSettled`, liberado por un `defer` de `AppBootstrapper.bootstrap()`, alimentando el blocker `bootstrapPending`— y NO el intent, que es lo que preserva el orden aviso→paywall. Gatea la ruta REAL: `UITestHooks` intactos. Verificado aquí en **iOS 27.0**, que es lo que faltaba y lo que importa (sus 11/11 en 26.5 prueban no-regresión, no cierre: en 26.x la tasa ya era 0/10): **9/9 verdes con el código de test SIN modificar**, cero `:88`, cero `:35`, y `Tap "fab_new_transaction"` exactamente 1 vez en las nueve ⇒ los 10 reintentos no se consumen. Revisadas y descartadas las dos formas en que el fix podría ser peor que el bug: el early return de `bootstrap()` precede al `defer`, pero solo es alcanzable tras una llamada que ya lo registró y nadie devuelve el flag a `false`; y el segundo releaser (`YalaApp.swift:125`) es el host de los unit tests, donde `bootstrap()` se salta a propósito — los UI tests no entran a ese guard | **CERRADO** |
| **0b** | **BLOQUEANTE DE RELEASE 2.1 — el sign-in con Google está roto en producción.** Verificado en el dashboard el 2026-07-28: en `yala-modo-nube-production` el provider **Apple está habilitado y Google NO** (se configuró solo en staging). Y Google no es un extra: `WelcomeAccountChoiceLogic.swift:61` lo mete en las opciones del **Welcome**, la primera pantalla de un usuario nuevo, y `StorageSignInChooserView.swift:70` lo ofrece en Ajustes. Un born-cloud (D-A7, la pieza central del valor de 2.1) que elija Google se estrella en el primer paso. **Lo que hace falta NO es solo «habilitar el provider»**: la app usa el flujo NATIVO (`CloudAuthService.swift:359`, `signInWithIdToken` con `provider: .google` y nonce), así que Supabase valida el `aud` del ID token contra su lista **Authorized Client IDs** — hay que habilitar el provider **y** añadir ahí el client ID de iOS. La buena noticia: `GID_CLIENT_ID` es **el mismo en las cuatro configuraciones** del `pbxproj` (un client ID de iOS identifica la app, no el backend) y staging ya funciona con él ⇒ es **copiar la configuración del provider de staging a producción**, sin dar de alta nada nuevo en Google Cloud | **ABIERTO — bloquea 2.1** |
...
| 2 | **Staging roto — el radio es MAYOR de lo que decía este documento, y la causa raíz no es la tabla.** La migración `0002_account_entitlements` se aplicó a producción y **no** a staging, y el código consulta `account_entitlements` **sin guardia** ⇒ en staging revientan no solo `register`/`assert` (`attest/routes.ts:87`) sino también los endpoints de entitlement (`sync/entitlement.ts:76` y `:87`), el borrado de cuenta (`sync/account.ts:240`) y la reconciliación del webhook de Apple (`proxy/webhook.ts:44-49`) — que además reintentaría para siempre contra una tabla inexistente. Es toda la superficie que 2.1 enciende, en el entorno donde se hace dogfooding. **CAUSA RAÍZ: no había ningún script de migración**, así que `0002` se aplicó a mano a un entorno y no al otro. Arreglado en el mismo commit con `migrate:staging` y `migrate:production` en `gateway/package.json`. La migración es idempotente (`CREATE TABLE IF NOT EXISTS` + `CREATE UNIQUE INDEX IF NOT EXISTS`), así que aplicarla es seguro. **CERRADO 2026-07-28**: `npm run migrate:staging` aplicada — `migrations list` pasa a «No migrations to apply», y verificado contra `sqlite_master` que existen la tabla `account_entitlements`, su `idx_account_entitlements_txn` y el autoindex del PK, con el `SELECT` del código respondiendo (0 filas). Los dos entornos del gateway vuelven a estar idénticos en schema | **CERRADO** |
...
| 4 | **Bloqueante #1 de 2.1 — INSPECCIONADO 2026-07-28, y es MÁS PEQUEÑO de lo que este documento decía.** `yala-modo-nube-production` (`kefvaiymtgytemwbltlz`) existe, está `ACTIVE_HEALTHY`, Postgres 17.6, y tiene **18 migraciones aplicadas** hasta `g10_01_transfer_group_ownership` (2026-07-21): el schema de Grupos está desplegado completo (g1 infra+RPCs · g2 delta · g3 meta/claim · g5 borrado personal · g6 `migrate_group` · g7 cifrado y cutover · g8 push fanout · g10 transferencia · g12 `auth.users` y lockdown de `stamp_group_seq`). **CORRECCIÓN: la anon key NO va por `Secrets.xcconfig`.** Este documento decía eso y era una aplicación indebida de la regla de CLAUDE.md, que apunta a **secretos**. La cabecera de `CloudBackendConfig.swift` documenta el patrón contrario **a propósito**: «commiteada, per-scheme, SIN secretos… la `anonKey` es PÚBLICA por diseño… NUNCA la `service_role`». Es una JWT de rol `anon` gobernada por RLS —clave *publicable*, misma clase que una config de Firebase— y la de staging ya está commiteada en `:34` con esa justificación. ⇒ **el cableado son DOS LÍNEAS**: rellenar la rama `#else` de `supabaseURL` y `anonKey`. **PITR verificado el 2026-07-28 en el dashboard: NO está activo** (add-on de pago sin contratar). Hay **backup diario físico** con ~7 días de retención — lo que además confirma plan **Pro**, no gratuito, así que el proyecto **no se autopausará** (el miedo venía de que `mundial-2026-data` de la misma cuenta está INACTIVE). Storage no aplica: la app no usa Supabase Storage, así que el aviso de «Storage objects are not included» es inocuo. **Decisión consciente de no contratar PITR ahora, con mitigación por retraso del borrado de D-A3 a 2 días** — ver la REVISIÓN 2026-07-28 en [[MODO-NUBE-DECISIONES-ESCENARIOS]] D-A3, que explica por qué el retraso es más barato y mejor que el PITR para el fallo probable. **Ojo con el orden**: esas dos líneas encienden de golpe toda la superficie de nube y auth en builds de producción, así que van DESPUÉS de #0, #1 y #2 | pendiente, 2 líneas + PITR |
