...
> **CORRECCIÓN (2026-07-15, verificada en código durante G3 — replicada al vault el 2026-07-15 tras destrabar iCloud):**
...
| R10 | Grupo MIGRADO en device fresco: FKs del historial (namespace CloudKit) vs ids born-remote (namespace backend) no matchean ⇒ balances rotos (hallazgo G3 2026-07-15, ver corrección §2 y §9.3d) | ✅ **DECIDIDO (owner 2026-07-16, arranque de G6): derivación NAMESPACE-AWARE en `applyMember`** — cuando el `member_key` es un recordName CloudKit legacy (distinguible del `sub` UUID), el apply deriva con el namespace CloudKit-era ⇒ el historial migra byte-idéntico, cero remap, devices preexistentes intactos. El remap en el uploader se DESCARTÓ: habría pisado las FKs del historial local de los devices preexistentes (sus members locales conservan ids CloudKit-era — clase IdentityRemap). |
