 ...
 * atiende y se descarta. Así no hay sesiones que guardar en el Worker, y el token del usuario vive lo que dura la
 * petición.
 */
...
export function buildServer(env: Env, token: VerifiedToken, deps: { fetcher?: Fetcher; now?: () => Date } = {}): McpServer {
  ...
  const reader = new YalaReader(env, token.raw, deps.fetcher);
  ...
  for (const def of TOOLS) {
    server.registerTool(
      ...
      async (args: Record<string, unknown>) => runTool(def, env, token, reader, args, deps.now),
    );
  }
  return server;
}
...
async function runTool(
  ...
  if (!planAllows(def.plan, { pro: false })) {
    audit({ tool: def.name, token, ms: 0, outcome: "plan" });
    return errorResult("Esta herramienta es de Yala Pro.");
  }
  try {
    const result = await def.run({ reader, env, now: now ? now() : new Date() }, args ?? {});
    audit({ tool: def.name, token, ms: Date.now() - started, outcome: "ok" });
    return { content: [{ type: "text" as const, text: JSON.stringify(result) }] };
  } catch (err) {
    if (err instanceof InvalidInputError) {
      audit({ tool: def.name, token, ms: Date.now() - started, outcome: "input" });
      return errorResult(err.message);
    }
    if (err instanceof UpstreamError) {
      audit({ tool: def.name, token, ms: Date.now() - started, outcome: `upstream_${err.status}` });
      return errorResult(
    ...
    audit({ tool: def.name, token, ms: Date.now() - started, outcome: "error" });
...
export async function handleMcp(
  ...
  const server = buildServer(env, token, deps);
  const transport = new WebStandardStreamableHTTPServerTransport({ sessionIdGenerator: undefined, enableJsonResponse: true });
  await server.connect(transport);
  try {
    return await transport.handleRequest(request, {
      authInfo: { token: token.raw, clientId: token.clientId, scopes: [], expiresAt: token.expiresAt },
    });
  } finally {
